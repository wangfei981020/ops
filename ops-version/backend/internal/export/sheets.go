...
func writeReadme(f *excelize.File, in Input, st *styles) error {
	...
	rows = append(rows, [][2]string{
		...
		{"不一致", "这几列都有这个服务，但 tag 不全相同。⚠️ 不说谁新谁旧 —— 没有基准列，而跨平台是两个 Harbor、两条流水线，版本号本来就不可比"},
		...
		{"同步状态未知", "复制规则没绑到这个平台 / 没配 Harbor / 还没拉取过记录。不等于没同步 —— 是我们不知道"},
		{"一致率分母", "排除「数据不可用」的列。拿不到数据不算不一致 —— 否则一个平台采集失败会让整张表看起来差异激增，掩盖真正的差异"},
	}...)
...
// 没有基准就没有"落后几个版本"，而跨平台两边是两个 Harbor、两条流水线，
// 版本号本来就不可比，那个数字在跨平台场景里是假的。
